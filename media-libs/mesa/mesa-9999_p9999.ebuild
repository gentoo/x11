...
if [[ ${PV} = 9999* ]]; then
	...
	IUSE_UNSTABLE="gallium +radeon-kms"
	RDEPEND_UNSTABLE=">=x11-libs/libdrm-9999[radeon-kms?]"
...
pkg_setup() {
	...
	if [[ -n ${EXPERIMENTAL} ]] && use radeon-kms; then
		if use gallium; then
			eerror "USE=gallium currently doesn't build with USE=radeon-rewrite"
			eerror "However that's m00t since radeon-rewrite uses Gallium anyway"
			die "Disable USE=gallium"
		fi
		EGIT_BRANCH="radeon-rewrite"
		EGIT_TREE="radeon-rewrite"
	fi
